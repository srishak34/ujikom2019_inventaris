...
class t_pengembalianController extends Controller
{
    public function index_operatorTransaksi()
    {
		...
		$no = 1;
		return view('users.operator.t_pengembalian', compact('d', 'no'));
    }

    public function detail_operatorTransaksi($id)
	{
		...
		return view('users.operator.t_pengembalian.detail', compact('d', 'inv', 'pn'));
	}
}
