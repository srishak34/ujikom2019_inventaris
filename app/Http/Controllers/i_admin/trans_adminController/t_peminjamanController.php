...
class t_peminjamanController extends Controller
{
	public function index_dataTransaksi()
	{
		...
		$no = 1;
		return view('users.admin.transaksi.t_peminjaman', compact('d', 'no'));
	}

	public function create_dataTransaksi(Request $req)
	{
		if ($req -> isMethod('post')) {
			...
			if ($i_find->jumlah <= -1) {
				...
				if ($pn -> save()) {
					...
					if ($d->save()) {
						...
						return redirect('/admin/trans_peminjaman');
					} else {
			...
			$p = Pegawai::get();
			$i = Inventaris::get();
			return view('users.admin.transaksi.t_peminjaman.create', compact('p', 'i'));
		}
	}

	public function kembalikan_dataTransaksi(Request $req, $id)
	{
		if ($req->isMethod('patch')) {
			...
			if ($inv->save()) {
				...
				if ($pn->save()) {
					...
					return redirect('/admin/trans_pengembalian');
				} else {
			...
			return view('users.admin.transaksi.t_peminjaman.kembalikan', compact('d', 'inv', 'pn'));
		}
	}
	...
	public function detail_dataTransaksi($id)
	{
		...
		return view('users.admin.transaksi.t_peminjaman.detail', compact('d', 'inv', 'pn'));
	}

	public function izinkan_dataTransaksi($id)
    {
        $date = new DateTime;
        $d = DetailP::findOrFail($id);
        $inv = Inventaris::findOrFail($d->inventaris_id);
